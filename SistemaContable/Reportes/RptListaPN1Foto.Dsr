VERSION 5.00
Begin {9EB8768B-CDFA-44DF-8F3E-857A8405E1DB} RptListaPN1Foto 
   Caption         =   "SIGC - RptListaPN1Foto (ActiveReport)"
   ClientHeight    =   10815
   ClientLeft      =   0
   ClientTop       =   285
   ClientWidth     =   20400
   _ExtentX        =   35983
   _ExtentY        =   19076
   SectionData     =   "RptListaPN1Foto.dsx":0000
End
Attribute VB_Name = "RptListaPN1Foto"
...
Private Sub Detail_Format()
   ...
   If txtaux_ctrl1 <> "X" Then
      ...
      Set Rst110 = Nothing
      Set Rst110 = New ADODB.Recordset
      If Rst110.State = 1 Then Rst110.Close
      Rst110.Open "Select * from ParametroFTP ", Cnn, adOpenKeyset, adLockReadOnly, adCmdText
   
      Set Rst50 = Nothing
      Set Rst50 = New ADODB.Recordset
      If Rst50.State = 1 Then Rst50.Close
      If Trim(Field2.Text) = "" Then
         Rst50.Open "Select IsNull(art_pathfoto,'') as art_pathfoto From Articulo Where art_codtex = '" & txtlis_codtexC1 & "' And art_codnum = " & txtlis_codnumC1, Cnn, adOpenKeyset, adLockReadOnly, adCmdText
      Else
         Rst50.Open "Select IsNUll(ada_pathfoto,'') as art_pathfoto From Articulo Left Join AdicionalxArtic on art_codtex = ada_codtex And art_codnum = ada_codnum " _
         + "Where art_codtex = '" & txtlis_codtexC1 & "' And art_codnum = " & txtlis_codnumC1 & " And ada_adicional = '" & Trim(Field2.Text) & "'", Cnn, adOpenKeyset, adLockReadOnly, adCmdText
      End If
      
      On Error Resume Next
      If Trim(Rst50!art_pathfoto) <> "" Then
         Foto.Visible = True
         Foto.Picture = LoadPicture(UCase(left(Rst110!par_CopiPath, 1)) & UCase(right(Rst50!art_pathfoto, Len(Rst50!art_pathfoto) - 1)))
         Detail.Height = Foto.Height
         If Err.Number <> 0 Then
            Err.Clear
            Foto.Visible = False
            Foto.Picture = Nothing
            Detail.Height = txtlis_codtexC1.Height
         End If
      Else
         Foto.Visible = False
         Foto.Picture = Nothing

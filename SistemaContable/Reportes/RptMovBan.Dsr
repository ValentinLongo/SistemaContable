VERSION 5.00
Begin {9EB8768B-CDFA-44DF-8F3E-857A8405E1DB} RptMovban 
   Caption         =   "SIGC - RptMovban (ActiveReport)"
   ClientHeight    =   10815
   ClientLeft      =   0
   ClientTop       =   285
   ClientWidth     =   15360
   _ExtentX        =   27093
   _ExtentY        =   19076
   SectionData     =   "RptMovBan.dsx":0000
End
Attribute VB_Name = "RptMovban"
...
Private Sub ActiveReport_ReportStart()
   ...
   If txt_saldo.Visible = True Then txt_saldo.Text = Format(CDec(txt_salini.Text), "0.00")
End Sub
...
On Error GoTo fin
   ...
   Menu.Dialog1.FileName = "Movimientos Bancarios.pdf"
...
On Error GoTo fin
   ...
   Menu.Dialog1.FileName = "Movimientos Bancarios.xls"
   On Error Resume Next
   Menu.Dialog1.CancelError = True
   Menu.Dialog1.ShowSave
   If Err.Number = 32755 Then
...
Private Sub Detail_BeforePrint()
   If txt_saldo.Visible = True Then txt_saldo.Text = Format(CDec(txt_saldo.Text) + (CDec(txtaux_debe.Text) - CDec(txtaux_haber.Text)), "0.00")
End Sub

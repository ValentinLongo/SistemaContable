VERSION 5.00
Begin {9EB8768B-CDFA-44DF-8F3E-857A8405E1DB} RptParamLista 
   Caption         =   "SIGC - RptParamLista (ActiveReport)"
   ClientHeight    =   10815
   ClientLeft      =   0
   ClientTop       =   285
   ClientWidth     =   15360
   _ExtentX        =   27093
   _ExtentY        =   19076
   SectionData     =   "RptParamLista.dsx":0000
End
Attribute VB_Name = "RptParamLista"
...
On Error GoTo fin
   ...
   Menu.Dialog1.FileName = "Historico de Listas.pdf"
...
On Error GoTo fin
   ...
   Menu.Dialog1.FileName = "Historico de Listas.xls"
   On Error Resume Next
   Menu.Dialog1.CancelError = True
   Menu.Dialog1.ShowSave
   If Err.Number = 32755 Then
...
Private Sub Detail_Format()
   If Trim(Field3.Text) = "" Then
      Field3.Text = "TODAS"
   End If
   If Trim(Field4.Text) = "" Then
      Field4.Text = "TODAS"
   End If
   If Trim(Field5.Text) = "" Then
      Field5.Text = "TODOS"
   End If
   If Trim(Field6.Text) = "" Then
      Field6.Text = "TODAS"
   End If
   If Trim(Field7.Text) = "" Then
      Field7.Text = "TODAS"
   End If
End Sub

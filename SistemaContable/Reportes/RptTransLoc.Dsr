VERSION 5.00
Begin {9EB8768B-CDFA-44DF-8F3E-857A8405E1DB} RptTransLoc 
   Caption         =   "SIGC - RptTransLoc (ActiveReport)"
   ClientHeight    =   10815
   ClientLeft      =   0
   ClientTop       =   285
   ClientWidth     =   15360
   _ExtentX        =   27093
   _ExtentY        =   19076
   SectionData     =   "RptTransLoc.dsx":0000
End
Attribute VB_Name = "RptTransLoc"
...
On Error GoTo fin
   ...
   Menu.Dialog1.FileName = "Localidades por Transporte.pdf"
...
On Error GoTo fin
   ...
   Menu.Dialog1.FileName = "Localidades por Transporte.xls"
   On Error Resume Next
   Menu.Dialog1.CancelError = True
   Menu.Dialog1.ShowSave
   If Err.Number = 32755 Then
...
Private Sub Detail_BeforePrint()
   If txttrl_lunes.Text = "1" Then
      txttrl_lunes.Text = "Si"
   Else
      txttrl_lunes.Text = ""
   End If

   If txttrl_martes.Text = "1" Then
      txttrl_martes.Text = "Si"
   Else
      txttrl_martes.Text = ""
   End If

   If txttrl_miercoles.Text = "1" Then
      txttrl_miercoles.Text = "Si"
   Else
      txttrl_miercoles.Text = ""
   End If

   If txttrl_jueves.Text = "1" Then
      txttrl_jueves.Text = "Si"
   Else
      txttrl_jueves.Text = ""
   End If

   If txttrl_viernes.Text = "1" Then
      txttrl_viernes.Text = "Si"
   Else
      txttrl_viernes.Text = ""
   End If

   If txttrl_sabado.Text = "1" Then
      txttrl_sabado.Text = "Si"
   Else
      txttrl_sabado.Text = ""
   End If

   If txttrl_domingo.Text = "1" Then
      txttrl_domingo.Text = "Si"
   Else
      txttrl_domingo.Text = ""
   End If

VERSION 5.00
Begin {9EB8768B-CDFA-44DF-8F3E-857A8405E1DB} RptTotPtoVta 
   Caption         =   "SIGC - RptTotPtoVta (ActiveReport)"
   ClientHeight    =   10815
   ClientLeft      =   0
   ClientTop       =   285
   ClientWidth     =   15360
   _ExtentX        =   27093
   _ExtentY        =   19076
   SectionData     =   "RptTotPtoVta.dsx":0000
End
Attribute VB_Name = "RptTotPtoVta"
...
On Error GoTo fin
   ...
   Menu.Dialog1.FileName = "Totales.pdf"
...
On Error GoTo fin
   ...
   Menu.Dialog1.FileName = "Totales.xls"
   On Error Resume Next
   Menu.Dialog1.CancelError = True
   Menu.Dialog1.ShowSave
   If Err.Number = 32755 Then
...
'   If txttmo_codigo.Text = 1 Or txttmo_codigo.Text = 3 Or txttmo_codigo.Text = 5 Then
'      Field2.Text = CDec(Field2.Text) + CDec(txtTot.Text)
'   ElseIf txttmo_codigo.Text = 2 Or txttmo_codigo.Text = 4 Then
'      Field2.Text = CDec(Field2.Text) - CDec(txtTot.Text)
'   End If
End Sub

Private Sub GroupHeader1_BeforePrint()
   txt_ptovta.Text = Format(txtpto_codigo.Text, "0000")
   Field2.Text = 0
End Sub

VERSION 5.00
Begin {9EB8768B-CDFA-44DF-8F3E-857A8405E1DB} RptMovBanDia 
   Caption         =   "SIGC - RptMovBanDia (ActiveReport)"
   ClientHeight    =   10815
   ClientLeft      =   0
   ClientTop       =   285
   ClientWidth     =   15360
   _ExtentX        =   27093
   _ExtentY        =   19076
   SectionData     =   "RptMovBanDia.dsx":0000
End
Attribute VB_Name = "RptMovBanDia"
...
On Error GoTo fin
   ...
   Menu.Dialog1.FileName = "Movimientos.pdf"
...
On Error GoTo fin
   ...
   Menu.Dialog1.FileName = "Movimientos.xls"
   On Error Resume Next
   Menu.Dialog1.CancelError = True
   Menu.Dialog1.ShowSave
   If Err.Number = 32755 Then
...
Private Sub Detail_BeforePrint()
   If txt_saldo.Visible = True Then txt_saldo.Text = Format(CDec(txt_saldo.Text) + (CDec(txtDebe.Text) - CDec(txtHaber.Text)), "0.00")
End Sub

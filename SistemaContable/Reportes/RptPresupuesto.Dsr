VERSION 5.00
Begin {9EB8768B-CDFA-44DF-8F3E-857A8405E1DB} RptPresupuesto 
   Caption         =   "RptPresupuesto (ActiveReport)"
   ClientHeight    =   10815
   ClientLeft      =   0
   ClientTop       =   285
   ClientWidth     =   15360
   _ExtentX        =   27093
   _ExtentY        =   19076
   SectionData     =   "RptPresupuesto.dsx":0000
End
Attribute VB_Name = "RptPresupuesto"
...
On Error GoTo fin
   ...
   Menu.Dialog1.FileName = "Presupuesto.pdf"
...
On Error GoTo fin
   ...
   Menu.Dialog1.FileName = "Presupuesto.xls"
   On Error Resume Next
   Menu.Dialog1.CancelError = True
   Menu.Dialog1.ShowSave
   If Err.Number = 32755 Then
...
Private Sub Detail_BeforePrint()
   On Error Resume Next
   If Not lbl_param1.Caption = "1" Then
      If txthis_codtex.Text = "0" And txthis_codnum = "88888888" Then
         txthis_codtex.Visible = False
         txthis_codnum.Visible = False
      Else
         txthis_codtex.Visible = True
         txthis_codnum.Visible = True
      End If
   End If
   ...
   If CDec(txthis_cantidad.Text) = 0 Then
      barra.Visible = True
   Else
      barra.Visible = False
   End If

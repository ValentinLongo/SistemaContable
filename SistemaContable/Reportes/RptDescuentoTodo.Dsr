VERSION 5.00
Begin {9EB8768B-CDFA-44DF-8F3E-857A8405E1DB} RptDescuentoTodo 
   Caption         =   "SIGC - RptDescuentoTodo (ActiveReport)"
   ClientHeight    =   10815
   ClientLeft      =   0
   ClientTop       =   285
   ClientWidth     =   15360
   _ExtentX        =   27093
   _ExtentY        =   19076
   SectionData     =   "RptDescuentoTodo.dsx":0000
End
Attribute VB_Name = "RptDescuentoTodo"
...
Private Sub GroupHeader1_BeforePrint()
   Select Case txtpra_secuencia.Text
      ...
      Case Is = 5
         secuencia.Caption = "Descuentos por Cliente"
   End Select
End Sub
...
On Error GoTo fin
   ...
   Menu.Dialog1.FileName = "Descuentos por Cliente.pdf"
...
On Error GoTo fin
   ...
   Menu.Dialog1.FileName = "Descuentos por Cliente.xls"
   On Error Resume Next
   Menu.Dialog1.CancelError = True
   Menu.Dialog1.ShowSave
   If Err.Number = 32755 Then

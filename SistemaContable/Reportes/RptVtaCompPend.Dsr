VERSION 5.00
Begin {9EB8768B-CDFA-44DF-8F3E-857A8405E1DB} RptVtaCompPend 
   Caption         =   "SIGC - RptVtaCompPend (ActiveReport)"
   ClientHeight    =   10815
   ClientLeft      =   0
   ClientTop       =   285
   ClientWidth     =   15360
   _ExtentX        =   27093
   _ExtentY        =   19076
   SectionData     =   "RptVtaCompPend.dsx":0000
End
Attribute VB_Name = "RptVtaCompPend"
...
On Error GoTo fin
   ...
   Menu.Dialog1.FileName = "Comprobantes.pdf"
...
Private Sub Detail_BeforePrint()
   If txtpnd_dias.Text = "0" Then txtpnd_dias.Text = ""
   
   acum.Text = Format(CDec(Acu), "0.00")
   acum.Text = Format(CDec(acum.Text) + CDec(txtpnd_saldo.Text), "0.00")
   Acu = CDec(acum.Text)
End Sub

VERSION 5.00
Begin {9EB8768B-CDFA-44DF-8F3E-857A8405E1DB} RptConsulCompIns 
   Caption         =   "SIGC - RptConsulCompIns (ActiveReport)"
   ClientHeight    =   10815
   ClientLeft      =   0
   ClientTop       =   285
   ClientWidth     =   15360
   _ExtentX        =   27093
   _ExtentY        =   19076
   SectionData     =   "RptConsulCompIns.dsx":0000
End
Attribute VB_Name = "RptConsulCompIns"
...
On Error GoTo fin
   ...
   Menu.Dialog1.FileName = "Insumos.pdf"
...
On Error GoTo fin
   ...
   Menu.Dialog1.FileName = "Insumos.xls"
   On Error Resume Next
   Menu.Dialog1.CancelError = True
   Menu.Dialog1.ShowSave
   If Err.Number = 32755 Then
...
Private Sub Detail_Format()
   If txtmvi_tipo.Text = "1" Then
      Field1.Text = "IMPORTE"
   Else
      Field1.Text = "PORCENTAJE"
   End If
   
   If txtmvi_aputil.Text = "1" Then
      Field2.Text = "SI"
   Else
      Field2.Text = "NO"
   End If

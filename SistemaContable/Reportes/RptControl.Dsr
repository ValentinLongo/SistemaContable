VERSION 5.00
Begin {9EB8768B-CDFA-44DF-8F3E-857A8405E1DB} RptControl 
   Caption         =   "SIGC - RptControl (ActiveReport)"
   ClientHeight    =   10815
   ClientLeft      =   0
   ClientTop       =   285
   ClientWidth     =   15360
   _ExtentX        =   27093
   _ExtentY        =   19076
   SectionData     =   "RptControl.dsx":0000
End
Attribute VB_Name = "RptControl"
...
On Error GoTo fin
   ...
   Menu.Dialog1.FileName = "Control.pdf"
...
On Error GoTo fin
   ...
   Menu.Dialog1.FileName = "Control.xls"
   On Error Resume Next
   Menu.Dialog1.CancelError = True
   Menu.Dialog1.ShowSave
   If Err.Number = 32755 Then
...
Private Sub Detail_BeforePrint()
   If txtctrl_fecusu.Text = "01/01/1900" Then
      txt_fecha = ""
      txt_hora = ""
   Else
      txt_fecha = txtctrl_fecusu.Text
      txt_hora = txtctr_hora.Text
   End If
   On Error Resume Next
   If txtCtrl.Text = "Autorizado" Then
      Image1.Picture = LoadPicture(App.Path & "\Tilde.bmp")
   Else

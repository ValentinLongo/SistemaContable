VERSION 5.00
Begin {9EB8768B-CDFA-44DF-8F3E-857A8405E1DB} RptComisionesVen 
   Caption         =   "RptComisionesVen (ActiveReport)"
   ClientHeight    =   10815
   ClientLeft      =   0
   ClientTop       =   285
   ClientWidth     =   15360
   _ExtentX        =   27093
   _ExtentY        =   19076
   SectionData     =   "RptComisionesVen.dsx":0000
End
Attribute VB_Name = "RptComisionesVen"
...
Private Sub GroupHeader1_BeforePrint()
   Select Case txtpra_secuencia.Text
      Case Is = 1
         secuencia.Caption = "Descuentos por Fábrica"
      Case Is = 2
         secuencia.Caption = "Descuentos por Fábrica y Línea"
      Case Is = 3
         secuencia.Caption = "Descuentos por Fábrica, Línea y Rubro"
      Case Is = 4
         secuencia.Caption = "Descuentos por Artículo"
      Case Is = 5
         secuencia.Caption = "Descuentos Generales"
   End Select
End Sub
...
On Error GoTo fin
   ...
   Menu.Dialog1.FileName = "Comisiones.pdf"
...
On Error GoTo fin
   ...
   On Error Resume Next
   Menu.Dialog1.CancelError = True
   Menu.Dialog1.FileName = "Comisiones.xls"
   Menu.Dialog1.ShowSave
   If Err.Number = 32755 Then
      Err.Clear
      Exit Sub

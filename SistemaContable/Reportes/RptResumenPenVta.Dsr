VERSION 5.00
Begin {9EB8768B-CDFA-44DF-8F3E-857A8405E1DB} RptResumenPenVta 
   Caption         =   "SIGC - RptResumenPenVta (ActiveReport)"
   ClientHeight    =   10815
   ClientLeft      =   0
   ClientTop       =   285
   ClientWidth     =   15360
   _ExtentX        =   27093
   _ExtentY        =   19076
   SectionData     =   "RptResumenPenVta.dsx":0000
End
Attribute VB_Name = "RptResumenPenVta"
...
On Error GoTo fin
   ...
   Menu.Dialog1.FileName = "Cpbtes Pendientes.pdf"
...
On Error GoTo fin
   ...
   Menu.Dialog1.FileName = "Cpbtes Pendientes.xls"
   On Error Resume Next
   Menu.Dialog1.CancelError = True
   Menu.Dialog1.ShowSave
   If Err.Number = 32755 Then
...
Private Sub ReportFooter_BeforePrint()
   lbl_salfin.Text = Format(CDec(IIf((txtpen_cancel.Text = ""), 0, txtpen_cancel.Text)), "0.00")
End Sub
